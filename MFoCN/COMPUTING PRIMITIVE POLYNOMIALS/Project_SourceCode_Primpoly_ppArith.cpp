...
IntType addMod( IntType a, IntType b, IntType n )
{
    ...
    // Assume the positive arguments might not already be in the range [0, n).
    a %= n ;
    b %= n ;
...
IntType timesTwoMod( IntType a, IntType n )
{
    ...
    // Assume the positive argument might not already be in the range [0, n).
    a %= n ;
    ...
    if (mask & a)
    {
        // See notes in function addMod() above for why this works correctly.
        c -= n ;
...
|     Throws ArithModPError if a < 1 or p < 2 or p is even.
...
/*=============================================================================
 ...
 |                      0                0
 |   of polynomial f(x) and a is the number from the orderR() test.
...
bool ArithModP::constCoeffTest( ppsint a, ppsint a0, int n )
{
    ...
    if (n % 2 != 0)
        constant_coeff = -constant_coeff ;    // (-1)^n < 0 for odd n.
