...
| DESCRIPTION
|
|     Class for arbitrary precision non-negative integer arithmetic using any base.
|
|     We support the basic operations               + - * /
|     Also the C-like operators                     += -= *= /= ++ --       where some have mixed BigInt and integer
|     Comparisions                                  < > <= >= == !=         where some have mixed BigInt and integer
|     I/O operations                                << >>
|     Conversion from ordinary integers, to ordinary integers (can throw an overflow exception)
|     Conversions to and from strings
|     Bit masking & shifting << bit test and num bits
|     power (not in BigInt class)
|     ceil( lg() )
|
...
class BigInt
{
    public:
        ...
        // Constructor from unsigned integers.
        // Use explicit on these one argument constructors to prevent the compiler doing any
        // implicit conversions from integers to BigInts.  This should have been the C++ language default.
        ...
        // e.g. BigInt u( "123" ) ; string s = u.toString() ;
